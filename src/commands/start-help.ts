  ...
  handler: async (ctx: BotContext) => {
    try {
      ...
      if (!existingUser) {
        ...
        await ctx.reply(
          `🤖 *Welcome to Rodeo Trading Bot!*\n\n` +
            `The vibe-trading platform for Base. Trade ERC-20 tokens with AI on Base.\n\n` +
            `🧱 *Getting Started*\n` +
        ...
        await ctx.reply(
          `🤖 *Welcome back to Rodeo Trading Bot!*\n\n` +
            `The vibe-trading platform for Base. What would you like to do today?`,
          {
            parse_mode: "Markdown",
